var ajaxLoader = {
    ...
    'init': function (option) {
        ...
        $('body').append(
            '<div class="alert_popup_overlay" id="ALP_ajax-overlay"' + 'style="background: none repeat scroll 0 0 #00000">' + '<div class="ALP_content">' + '<div class="alp_loader">Loading...</div>' + '</div></div>');
        ...
        $('.alp_loader').css('margin', 'auto');
        if (option.size != 'empty' && option.size > 0 && option.size < 75) {
            $('.alp_loader').css('font-size', option.size + 'px');
        }
