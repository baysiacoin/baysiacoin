...
class Wallet extends Model {
     ...
     * @var array
     */
    protected $fillable = ['id', 'type', 'name', 'wallet_username', 'wallet_password', 'wallet_ip', 'port', 'download_wallet_client', 'logo_coin', 'is_moneypaper', 'limit_confirmations', 'enable_deposit', 'enable_withdraw'];
     ...
     */
    public function getIDFromCurrency($currency)
    {
        if (empty($currency)) {
            return 0;
        }
        $wallet = Wallet::where('type', '=', $currency)->first();
        return $wallet->id;
    }
